        ...
        <div className="mt-14 grid items-stretch gap-8 lg:grid-cols-2">
          {workflows.map((wf) => (
            <article key={wf.title} className="flex flex-col rounded-[20px] bg-plum p-8 text-white sm:p-10">
              <h3 className="text-base font-bold text-white/80">{wf.tag}</h3>
              <p className="mt-2 font-sans text-3xl font-bold leading-tight text-white">
                {wf.title}
              </p>
              <p className="mt-4 text-justify text-base leading-relaxed text-white/85">{wf.body}</p>
